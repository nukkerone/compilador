...
import java.util.*;
import assembler.DireccionRepreVarAssemble;
...
static int CANTREG = 4;
	/**
	 * Dado un terceto o una varaible, saber en que registro está, si es que está..
	 */
	...
	private void ubicarEnRegNum(int nreg, Typeable t) {
		...
		Direccion d = ubicar(t); //encontrar donde estaba
	...
	public Direccion ubicarEnInmed(Typeable t) {
		if(t instanceof ConstanteEntera)
			return new Inmediate((ConstanteEntera) t);
		else
			return null;
	}
	
	public Direccion ubicarEnRegistroOInmed(Typeable t) {
		Direccion d = this.ubicarEnInmed(t);
		if(d == null)
	...
	/**
	 * ubica el tipable en cualquier lugar
	 * @param t tipable que quiere ubicar
	 * @return ubicacion del tipable
	 */
	public Direccion ubicar(Typeable t) {
	...
	public Direccion ubicarEnInmedOMemoria(Typeable t) {
	...
	public Direccion ubicarEnRegistroOMemoria(Typeable t) {
		Direccion d = null;
		if(estaEnRegistro(t))
			d = getRegistro(t);
		else
	...
	public Vector<String> getVariables(){
		...
		while (e.hasMoreElements()) {
			...
			if (variable.getTk().getTipo() == Typeable.TIPO_UINT)
				salida.add(variable.getNombre()+" DW "+variable.getTk().getInitialValue());
			else
				if (variable.getTk().getTipo() == Typeable.TIPO_ULONG)
